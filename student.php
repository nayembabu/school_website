...
if (isset($_GET['sroll']) && isset($_GET['sclass'])) {
    ...
    try {
        // Prepare SQL query
        $stmt = $pdo->prepare("SELECT * FROM student_lists WHERE std_roll_no = :sroll AND std_class_autunq_id = :sclass");
        $stmt->bindParam(':sroll', $sroll, PDO::PARAM_INT);
        $stmt->bindParam(':sclass', $sclass, PDO::PARAM_INT);
        $stmt->execute();
              ...
              <ul class="menu">
                ...
                <li class="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    ...
                    <li class="active"><a href="teacher-details.html">Teacher Details</a></li>
                  </ul>
                </li>
                <li class="dropdown">
                  <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
...
<!-- Student Details finder -->
<section class="Student_details">
    <div class="Student_searchArea">
        <form id="searchForm">
            <div>
              <label for="sroll">Enter Student Roll Number:</label><br>
              <input type="number" name="sroll" id="sroll" required>
            </div>
            <div>
              <label for="sclass">Select Student Class</label><br>
    ...
    <div class="StudentInfo_showArea" id="result">
...
document.getElementById('searchForm').addEventListener('submit', function(event) {
    ...
    fetch(`student.php?sroll=${sroll}&sclass=${sclass}`)
        ...
        .then(data => {
            ...
            if (data.error) {
                ...
                resultArea.innerHTML = `
                    ...
                    <p><strong>Name:</strong> ${data.student_names}</p>
                    <p><strong>Father's Name:</strong> ${data.std_father}</p>
                    <p><strong>Mother's Name:</strong> ${data.std_mother}</p>
                    <p><strong>Class:</strong> ${data.std_class_autunq_id}</p>
                    <p><strong>Roll No:</strong> ${data.std_roll_no}</p>
                    <p><strong>Birth Reg No:</strong> ${data.std_birth_reg_no}</p>
                    <p><strong>Date of Birth:</strong> ${data.std_date_of_birth}</p>
                    <p><strong>Guardian Mobile No:</strong> ${data.gardian_mobile_no}</p>
                    <p><strong>Address:</strong> ${data.std_full_address}</p>
                `;
            }
        })
        .catch(error => {
            console.error("Parsing Error:", error);
            resultArea.innerHTML = `<p style="color: red;">Error fetching data. Check console for details.</p>`;
        });
});
</script>
</body>
